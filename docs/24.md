...
通过回归效应，你知道答案必须小于50%。如果![](http://latex.codecogs.com/gif.latex?Y^*)表示随机选取的儿子的身高(标准单位)，![](http://latex.codecogs.com/gif.latex?X^*)表示他的父亲的身高(标准单位)，则比例近似为
...
通常，您想要以测量数据的单位进行预测。在改变上述公式中的单位之前，请记住$X$上的条件等价于![](http://latex.codecogs.com/gif.latex?X^*)上的条件。如果你知道X或者![](http://latex.codecogs.com/gif.latex?X^*)的值，你也知道另一个。
