...
令$X$和$Z$是独立的标准正态变量，即具有均值向量$0$和协方差矩阵等于单位矩阵的二元正态随机变量。 现在确定一个数$\rho$（即希腊字母rho，小写r），使

![](http://latex.codecogs.com/gif.latex?-1<\rho<1)，并令![](http://latex.codecogs.com/gif.latex?A=\left[\begin{matrix}1&0\\\rho&\sqrt{1-\rho{^2}}\\\end{matrix}\right])
