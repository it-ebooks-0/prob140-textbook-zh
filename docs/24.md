...
![](http://latex.codecogs.com/gif.latex?slope_of_regression_line=\frac{Cov(X,Y)}{Var(X)}=\tau{_X,_Y}\frac{\sigma{_Y}}{\sigma{_X}})

![](http://latex.codecogs.com/gif.latex?intercept_of_regression_line=E(Y)-slope\cdot{E(X)})
