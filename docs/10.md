...
马尔科夫链，以[安德烈·马尔科夫](https://en.wikipedia.org/wiki/Andrey_Markov)的名字命名的一类随机过程。一个非正式的描述，对于马尔科夫链而言，将来的状态的值只取决于当前状态的值，而与如何达到当前状态的值无关。这一现象被称为*马尔科夫性质*。从形式上看

- 对于任一$n \ge 1$， $X_{n+1}$ 的条件分布，给出的 $X_0, X_1, \ldots , X_n$中，只取决于$X_n$
...
设 $X_{0} = a$，对于 $n > 0$ 令 $X_{n+1} = X_n + I_n$，其中$I_1, I_2, \ldots $ 是伯努利实验中的独立同分布序列。马尔科夫性质适用于整个过程：给出赌徒在时刻$n$的财富数，那他在时刻$n+1$时的财富数的取值概率只与其在时刻$n$时的财富数有关，与$n$之前无关。所以该过程$X_0, X_1, X_2, \ldots $是一个马尔科夫链，代表着赌徒的财富随时间的演变。

马尔可夫链的*状态空间*是链中状态可能值的集合。上述随机游走的状态空间是所有整数的集合。在本课程中，我们将状态空间限制为离散的且通常是有限的。


### 条件独立 ###
两个随机变量 $X$和 $Y$是相互独立是指，$X$在条件$Y$的情况的条件分布和不在$Y$的情况下的条件分布是一致的。   
随机变量 $X$和$Y$ *相对于$Z$条件独立* 是指，$X$在条件$Y$和$Z$的情况下的条件分布和在条件$Z$的情况下的条件分布式一致的。也就是说，$Z$这一关于$Y$的额外知识，不会影响$X$的值。

在马尔可夫链中，如果定义时刻$n$ 为现在，定义时刻$n+1$为未来，时刻序列 $0$ 到 $n-1$  作为过去，马尔科夫性质表示过去和未来是条件独立的。
...
### 一阶转移矩阵 ###
链的*一阶转移矩阵*描述如下，矩阵 $\mathbb{P}$，其中$(i, j)$th元素是 $P(i, j) = P(X_1 = j \mid X_0 = i)$。

通常，$\mathbb{P}$简称为*转移矩阵*。请注意两个重要属性：
...
### 粘性反转随机漫步 ###
通常，马尔可夫链的转移行为更容易在*转移图*而不是矩阵中描述。下面是状态1,2,3,4和5上的链的图。该图显示了一阶转移概率。
...
为了找到从状态 $i$转移到$j$的概率，只需找矩阵$j$行$j$列的值即可。
...
$n$-阶转移概率$P_n(i, j)$ 可以使用 $n$-阶转移矩阵的 $(i, j)$th元素表示。对于任意状态 $i$，$n$-阶转移矩阵的第 $i$行包含了从状态$i$开始的链的条件分布$X_n$
...
如上结果为$\mathbb{P} \times \mathbb{P} = \mathbb{P}^2$矩阵的$(i, j)$th 元素。因此，二阶转移矩阵为$\mathbb{P}^2$。
...
这是一个显示，但要使用矩阵，我们必须以Python识别为矩阵的形式表示它。方法`get_transition_matrix`为我们做到了这一点。需要步数$n$作为入参，并以numpy矩阵的格式返回$n$-阶转移矩阵。
...
如果链的所有状态彼此连通，则该链被称为*不可简化*。

上一节的粘性反转随机漫步是不可简化的，因为链条可能从每个状态相互之间都是连通的。
...
周期会导致长期行为的描述出现问题。例如：当状态$i$有周期3,序列$P_n(i, i)$可能开起来像"0, 0, positive, 0, 0, positive, $\ldots$"，因此限制声明可能会变得复杂。

在本课程中，我们将研究链条的长期行为，其中所有状态都是*非周期性的*，即它们具有周期1.换句话说，链条上无环。

你如何检查所有状态是否具有周期性？如果链是不可简化的，那所有状态必须具有相同的周期。这个的证据并不困难，但我们不会这样做。因为这意味着如果一个链是不可简化的，这很容易检查，你所要做的就是找出其中一个状态的周期。然后所有其他状态必须有相同的周期。
...
在本课程中，我们将只使用*有限状态空间上的不可简化的非周期马尔可夫链*。我们所说的大部分内容也适用于周期链，以及具有可数无限状态空间的链。
...
每个具有有限状态空间的不可简化和非周期性的马尔可夫链在状态转移一段时间后会表现出惊人的规律性。以下收敛定理的证明超出了本课程的范围，但您已经通过计算看到了结果。对于某些类别无限多个状态的马氏链，所有结果都更为正确。


### 收敛性 ###
设$X_0, X_1, \ldots$ 是有限状态空间$S$上的不可简化，非周期性的马尔科夫链。那么对于所有的状态 $i$ 和 $j$有
...
### 唯一性 ###
不难表明,如果平衡方程有解，那么它必须是$\pi$，$X_n$的边际分布的极限。我们不会做证明; 它基本上重复了我们用来推导平衡方程的步骤。你应该意识到，一个不可约的，非周期的，有限状态马尔可夫链只有一个静态分布。

如果您碰巧猜测到平衡方程的解，这将特别有用。如果您猜到了一个概率分布解，那么您已经找到了链的静态分布。
...
该模型说有两个容器总共含有 $N$粒子。在每个瞬间，随机选择容器，并且独立于容器随机选择颗粒。然后将所选粒子放入所选容器中; 如果它已经在那个容器中，那就留在那里。
...
这条链显然是不可简化的。它是非周期性的，因为 $P(i, i) > 0$.
